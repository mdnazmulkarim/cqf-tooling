...
import org.opencds.cqf.library.stu3.InMemoryLibraryResourceProvider;
import org.opencds.cqf.library.stu3.NarrativeProvider;
import org.opencds.cqf.measure.stu3.CqfMeasure;
import org.opencds.cqf.measure.stu3.DataRequirementsProvider;
...
public class HqmfUtils {
    ...
    public static Map<String, CodeMapping> measurePopulationValueSetMap = new HashMap<>() {
        {
    ...
    private void addDefinitions(XMLBuilder2 xml, CqfMeasure m) {
        ...
        if (m.hasTerminology())
        {
            for (CqfMeasure.TerminologyRef t : m.getTerminology()) {
                if (t.getType() == CqfMeasure.TerminologyRef.TerminologyRefType.VALUESET) {
                    this.addValueSet(xml, t);
                }
            }

            for (CqfMeasure.TerminologyRef t : m.getTerminology()) {
                if (t.getType() == CqfMeasure.TerminologyRef.TerminologyRefType.CODE) {
                    this.addDirectReferenceCode(xml, (CqfMeasure.CodeTerminologyRef)t);
                }
            }
        }
    }

    private void addValueSet(XMLBuilder2 xml, CqfMeasure.TerminologyRef t) {
        xml.root().elem("definition").elem("valueSet").a("classCode", "OBS").a("moodCode", "DEF").elem("id")
                .a("root", t.getId()).up()
                .elem("title").a("value", t.getName());
    }

    private void addDirectReferenceCode(XMLBuilder2 xml, CqfMeasure.CodeTerminologyRef t) {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            HqmfUtils provider = new HqmfUtils();
            ...
            List<IBaseResource> resources = paths.stream()
                    .map(HqmfUtils::toReader)
                    .filter(Objects::nonNull)
                    .map(parser::parseResource)
                    .collect(Collectors.toList());

            Measure measure = (Measure)resources.stream().filter(x -> (x instanceof Measure)).findFirst().orElseThrow();
            ...
            Path pathToHTML = Paths.get(Objects.requireNonNull(toUri("narratives/templates/hqmf.html")));
            ...
            writer = new PrintWriter(new File(pathToNarrativeOutput.toString()), StandardCharsets.UTF_8);
    ...
    public static URI toUri(String s) {
        try {
            return Objects.requireNonNull(HqmfUtils.class.getClassLoader().getResource(s)).toURI();
        }
        catch (Exception e) {
            e.printStackTrace();
            return null;
