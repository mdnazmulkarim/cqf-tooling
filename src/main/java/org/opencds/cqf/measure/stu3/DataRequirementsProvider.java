...
import org.opencds.cqf.utilities.HqmfUtils;
import org.opencds.cqf.utilities.QdmUtils;
import org.opencds.cqf.utilities.TranslatorUtils;
//import org.opencds.cqf.cql.execution.LibraryLoader;
...
public class DataRequirementsProvider {
    ...
    public CqfMeasure createCqfMeasure(Measure measure, LibraryResourceProvider libraryResourceProvider) {
        Map<VersionedIdentifier, Pair<org.cqframework.cql.elm.execution.Library, Library>> libraryMap = this.createLibraryMap(measure, libraryResourceProvider);
        return this.createCqfMeasure(measure, libraryMap);
    }

    private Map<VersionedIdentifier, Pair<org.cqframework.cql.elm.execution.Library, Library>>  createLibraryMap(Measure measure, LibraryResourceProvider libraryResourceProvider) {
        ...
        Map<VersionedIdentifier, Pair<org.cqframework.cql.elm.execution.Library, Library>> libraryMap = new HashMap<>();
    ...
    private CqfMeasure createCqfMeasure(Measure measure, Map<VersionedIdentifier, Pair<org.cqframework.cql.elm.execution.Library, Library>> libraryMap)
    {
        ...
        for (RelatedArtifact citation : cqfMeasure.getRelatedArtifact()) {
            if (citation.hasType() && citation.getType().toCode().equals("citation") && citation.hasCitation()) {
        ...
        org.cqframework.cql.elm.execution.Library primaryLibrary = libraryMap.values().stream()
        ...
        for (Entry<VersionedIdentifier, Pair<org.cqframework.cql.elm.execution.Library, Library>> libraryEntry : libraryMap.entrySet()) {
            org.cqframework.cql.elm.execution.Library library = libraryEntry.getValue().getLeft();
            ...
            if (library.getCodeSystems() != null && library.getCodeSystems().getDef() != null) {
                for (CodeSystemDef codeSystem : library.getCodeSystems().getDef()) {
                    ...
                    CqfMeasure.TerminologyRef term = new CqfMeasure.VersionedTerminologyRef(CqfMeasure.TerminologyRef.TerminologyRefType.CODESYSTEM, name, codeId, version);
            ...
            if (library.getCodes() != null && library.getCodes().getDef() != null) {
                for (CodeDef code : library.getCodes().getDef()) {
                    ...
                    for (CqfMeasure.TerminologyRef rf : codeSystems)
                    {
            ...
            if (library.getValueSets() != null && library.getValueSets().getDef() != null) {
                for (ValueSetDef valueSet : library.getValueSets().getDef()) {
                    ...
                    CqfMeasure.TerminologyRef term = new CqfMeasure.VersionedTerminologyRef(CqfMeasure.TerminologyRef.TerminologyRefType.VALUESET, name, valueSetId);
                    boolean exists = false;
                    ...
                    for (DataRequirement data : cqfMeasure.getDataRequirement()) {
                        ...
                        try {
                            QdmUtils.DataElementType dataType = QdmUtils.DataElementType.valueOf(type.toUpperCase());
                    ...
                    for (MeasureGroupComponent group : populationStatements) {
                        for (MeasureGroupPopulationComponent population : group.getPopulation()) {
                            if (population.getCriteria() != null && population.getCriteria().equalsIgnoreCase(statement.getName())) {
                                ...
                                String display = HqmfUtils.measurePopulationValueSetMap.get(code).displayName;
        ...
        Comparator<CqfMeasure.TerminologyRef> terminologyRefComparator = (item, t1) -> {
            String s1 = item.getDefinition();
            String s2 = t1.getDefinition();
            return s1.compareToIgnoreCase(s2);
        };
        Comparator<MeasureGroupPopulationComponent> populationComparator = (item, t1) -> {
    ...
    public CqlTranslator getTranslator(Library library, LibraryManager libraryManager, ModelManager modelManager) {
        ...
        return TranslatorUtils.getTranslator(
